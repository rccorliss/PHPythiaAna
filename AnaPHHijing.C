...
#include <AnaPHHijing.h>

using namespace std;

AnaPHHijing::AnaPHHijing(const std::string &name): SubsysReco(name)
{
  phpythia = 0;		// array of pythia (or hijing) particles
  phhijingheader = 0;	// hijing header
}

AnaPHHijing::~AnaPHHijing()
{
}

// Done at the beginning of processing
int AnaPHHijing::InitRun(PHCompositeNode *topNode)
{
  return EVENT_OK;
}

// Done at the end of processing
int AnaPHHijing::End(PHCompositeNode *topNode)
{
  return EVENT_OK;
}

// Process each event
int AnaPHHijing::process_event(PHCompositeNode *topNode)
{
  // Get PYTHIA Header
  phhijingheader = findNode::getClass<PHHijingHeader>(topNode,"PHHijingHeader");
  if (!phhijingheader)
    {
  ...
  cout << "Event: " << phhijingheader->GetEvt() << "\t" << phhijingheader->GetNpart() << endl;
